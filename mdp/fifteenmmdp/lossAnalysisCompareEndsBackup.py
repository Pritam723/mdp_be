    ...
    while(block <= endBlock) :
        ...
        if(parameter == "Percentage") :
            if(p_violation['status']) :
                ...
                violationInfo.append(f"Percentage Difference at {currentTS} is {p_violation['diff']}%.")
        elif(parameter == "MWH") :
            if(m_violation['status']) :
                violationCount = violationCount + 1
                violationInfo.append(f"MWH Difference at {currentTS} is {m_violation['diff']} MWH.")
        elif(parameter == "Percentage & MWH") :
            if(p_violation['status'] and m_violation['status']) :
                violationCount = violationCount + 1
                violationInfo.append(f"Percentage Difference at {currentTS} is {p_violation['diff']}%. MWH Difference at {currentTS} is {m_violation['diff']} MWH.")
        else :
...
def singleDayViolations(dateObject, end1, end2, fromTime, toTime, violations,parameter,percentageDiff,mwhDiff, path) :    
    # print(dateObject)
    ...
    try :
        ...
        violationCount, violationInfo = findViolations(df1, df2, fromTime, toTime, parameter, percentageDiff, mwhDiff)
        
        if(violationCount > 0) :
            violations[dateStr] = {'violationMsg' : f"There is/are {violationCount} violation(s) on this date (From {fromTime} to {toTime})", 'violationInfo' : violationInfo}
        
    except FileNotFoundError :
        dataNonAvailability = True
        violations[dateStr] = {'violationMsg' : "Data of atleast one End is not available for this date", 'violationInfo' : []}

    # print(dataNonAvailability)


def lossAnalysisCompareEnds(request, meter_id, path):
    ...
    mwhDiff = '0.2'
    startDate = '06/19/2023 00:00:00'
    endDate = '06/21/2023 23:45:00'
    ...
    for line in lineDetails :
        ...
        # print(violations)
        if(len(violations) > 0) :
            lineWiseViolationDetails.append({'feederName' : feederName, 'end1' : end1, 'end2' : end2, 'datewiseViolations' : violations})
